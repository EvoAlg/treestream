...
**Text File**\
A file whose contents can be fully decoded using Python's UTF-8 codec
with the strict error handler. All validly encoded Unicode characters
are permitted, including control characters, null bytes (U+0000), and
the Unicode replacement character (U+FFFD), provided they are validly
encoded as UTF-8. Files that cannot be decoded under these conditions
are not Text Files.\
...
-   The Serialized File shall be rejected if it contains two or more PATH values that are identical under case-insensitive Unicode comparison. Such paths would collide on the Windows filesystem and cannot be safely reconstructed.
...
A file contains bytes that cannot be decoded using Python's UTF-8 codec
with the strict error handler.
...
The serializer shall write output to a temporary file in the same directory as the designated output file. Upon successful completion of serialization, the temporary file shall be renamed to the designated output file, replacing any existing file at that location. If serialization fails for any reason, the temporary file shall be deleted and the designated output file shall not be created or modified.
...
**E10 --- Target Write Error**\
The system is unable to create directories or write files in the Target Directory, including due to insufficient permissions or path length limitations imposed by the Windows environment.
