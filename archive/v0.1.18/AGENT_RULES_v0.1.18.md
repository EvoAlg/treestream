...
**May not** use `TEST_REPORT.md`, `REVIEW.md`, or `IMPLEMENTATION_REQUEST.md`
...
**May not** modify `IMPLEMENTATION_REQUEST.md`, `TEST_REPORT.md`, or `REVIEW.md`.
...
Archive version numbers are per-file, not project-wide. Each governed
file (SPEC.md, SCENARIOS.feature, ACCEPTANCE_CRITERIA.md, AGENT_RULES.md)
maintains its own independent version history. Gaps in archive version
numbers for a given file are expected and are not governance violations.
...
- Content in `IMPLEMENTATION_REQUEST.md` or any other repository file,
